...
BankAccount ConsoleView::getAccountCreationData(int accountNumber) const {
    ...
    double initialBalance = getDoubleInput("Enter initial deposit (minimum $0): ");

    return BankAccount(accountNumber, initialBalance, age, name, address, password);
}
...
std::string ConsoleView::getNewPassword() const {
    return getPasswordInput("Enter new password (or press Enter to keep current): ");
}
